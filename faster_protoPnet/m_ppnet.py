...
class PPNet(nn.Module):
    ...
    def __init__(self, classifier,features, img_size, prototype_shape,
                 proto_layer_rf_info, num_classes,push_iter,proto_rf_boxes,proto_bound_boxes,init_weights=True,
        ...
        self.push_iter=0
        self.proto_rf_boxes=np.full(shape=[self.num_prototypes, 6],
                                    fill_value=-1)
        self.proto_bound_boxes=np.full(shape=[self.num_prototypes, 6],
                                    fill_value=-1)
    ...
    def forward(self, x):
        ...
        # conv_features = self.conv_features(x)
        # logits=self.alternative_head(x)
...
def construct_PPNet(classifier,base_architecture, pretrained=True, img_size=1224,
    ...
    return PPNet(classifier,features=features,
                 img_size=img_size,
