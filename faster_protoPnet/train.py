"""_____________________________Train Model______________________________________________
- to run the Exlanations - copy the LocalAnalysus_3.py script to train.py """
...
def eval(dataloader, faster_rcnn, trainer, test_num=2757):
    ...
    for ii, (img, sizes, gt_bboxes_, gt_labels_) in tqdm(enumerate(dataloader)):
        ...
        #pred_bboxes_, pred_labels_, pred_scores_ ,a,b,_,_= faster_rcnn.predict(img,visualize = True) #tirar sizes
        #pred_bboxes_, pred_labels_, pred_scores_ ,a,b,_,_= faster_rcnn.predict(img, sizes, visualize = True) #tirar sizes
        # if len(gt_labels_)==0:
        #     continue
        # o_img=img
        # img = inverse_normalize(at.tonumpy(img[0]))
        
        pred_bboxes_, pred_labels_, pred_scores_ ,a,b,_,_= faster_rcnn.predict(img, [sizes])#antes (img, [sizes]) e descom os sizes visualiza = false
...
def train(**kwargs):
    ...
    # best_path="/home/up202003072/Documents/faster_protoPnet/checkpoints/fasterrcnn_06282136_0.5586111118938318" #alterar
