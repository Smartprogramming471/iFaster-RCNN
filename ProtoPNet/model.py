...
class PPNet(nn.Module):
    ...
    def conv_features(self, x):
        ...
        # x = self.features(x)
        # b = np.zeros([7, 7]).astype(int)
        # left=int(bboxes[0]*1/19)
        # top=int(bboxes[1]*1/19)
        # right=int((bboxes[2]+bboxes[0])*1/19)
        # bottom=int((bboxes[3]+bboxes[1])*1/19)
        # bottom=7
        # b[top: bottom,left:right]=1
        # d=x.cpu().detach().numpy()*b
        # x = self.add_on_layers(x)
        return x
    ...
    def forward(self, x):
        ...
        #print(prototype_activations.shape) #20,30
...
def construct_PPNet(base_architecture, pretrained=True, img_size=224,
                    prototype_shape=(30, 128, 1, 1), num_classes=3,
                    prototype_activation_function='log',
                    add_on_layers_type='bottleneck'):
